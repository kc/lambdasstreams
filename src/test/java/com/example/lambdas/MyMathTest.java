...
public class MyMathTest {
    ...
    public void test() {
        ...
        // gevallen direct een referentie naar die methode meegeven ipv de lambda die hem aanroept

        MyMath myMath = new MyMath(42);

        // a method reference to a static method
        int execute1 = execute(4, (int a) -> a + 1);
        int execute2 = execute(4, (int a) -> MyMath.x(a)); // apply x on 4
        ...
        int value = 100;
        MyMath apply = f.apply(value);
        assertThat(apply.i, is(value));
    }

    private int execute(int i, IntFunction<Integer> c) {
        return c.apply(i);
    }
    ...
    private <T, U> T execute(T instance, U arg, BiFunction<T, U, T> f) {
        return f.apply(instance, arg);
    }

    private void execute(int i, IntConsumer c) {
        c.accept(i);
    }
}